...
```mermaid
flowchart LR
    A[Strategy and outcomes] --> B[Prioritised portfolio]
    B --> C[Architecture and assurance]
    C --> D[Iterative delivery]
    D --> E[Readiness and adoption]
    E --> F[Live service and value]
    F --> B
```
...
```mermaid
flowchart TD
    A[Opportunity] --> B[Risk and pathway decision]
    B --> C[Design and controls]
    C --> D[Build and test evidence]
    D --> E[Go-live decision]
    E --> F[Monitor and review]
    F --> G[Improve or retire]
```
